"""Consumer A CLI: parse arguments, call ConsumerService, print, exit code.
...
from consumer_a.observability import StageTimer, configure_logging, log_event
...
def main(argv: list[str] | None = None) -> int:
    """Run the Consumer A CLI."""

    parser = argparse.ArgumentParser(prog="consumer_a")
    subparsers = parser.add_subparsers(dest="command", required=True)

    ask_parser = subparsers.add_parser("ask", help="Answer a question from the OKF bundle.")
    ask_parser.add_argument("question", nargs="+", help="Natural-language question.")
    ask_parser.add_argument("--json", dest="json_output", action="store_true")
    ask_parser.add_argument("--max-docs", type=int, default=None)
    ...
    try:
        ...
        report = ConsumerService(config).answer(" ".join(args.question), max_docs=args.max_docs)
        ...
        print(f"✖ {exc}")
        log_event("ask.failed", error_type=type(exc).__name__)
        return exit_code_for(exc)
    except Exception:
        logging.getLogger("consumer_a").debug("unexpected internal error", exc_info=True)
